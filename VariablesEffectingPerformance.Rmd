...
```{r, fig.width = 15, fig.height = 10,  echo = FALSE, warnings = FALSE, message=FALSE, error=FALSE}
...
DownCorMat = Test_DF_Correlations( downdata[-r,-c(1,12:13)] ) 

##
rownames(DownCorMat[[1]]) = colnames(DownCorMat[[1]]) = names( downdata )[-c(1,12:13)]
rownames(DownCorMat[[2]]) = colnames(DownCorMat[[2]]) = names( downdata )[-c(1,12:13)]
...
```{r, echo = FALSE, warnings = FALSE, message=FALSE, error=FALSE, fig.width = 15, fig.height = 10}
...
## Extracting and plotting only the correlations for capture sensitivity @DP1

```{r, echo = FALSE, warnings = FALSE, message=FALSE, error=FALSE, fig.width = 15, fig.height = 8}
x = DownCorMat[[1]][, "DP1" ]
x = sort(x)
x = data.frame(CS = x)
...
par(mar = c(15, 10,3,3))
moose_barplot(data = x, column_to_plot = "CS", labels_vec = rownames(x) , rot_angle = 45, pcol = "blue", pmain = "Down Sampled Data (1.5M ProductionReads): Capture Sensitivity @DP1 Correlations")
dev.off()
...
## Explicity Univariate Analysis on complete data set:
...
## Explicity Univariate Analysis on Downsampled data including only those samples with 1.5M Production Reads:
...
## Multivariate model of the Full Data Set
...
### Multivariate model of the downsampled data set limited to only those samples with 1.5M production reads
...
#####
d = data.frame( d = MultivarMat_DOWNSAM["DP1",  c(4:10)] )
...
#####
d = data.frame( d = MultivarMat_DOWNSAM["DP1", c(20:25)  ] )
...
```{r, message = FALSE, warning = FALSE, error=FALSE, fig.width = 8, fig.height = 6}
## OK THIS IS SOME DAMN UGLY CODE !!!

## univariate etasq
u = UnivarMat_DOWNSAM$DP1[, 1]
u = u[c(3,2,1,4:7,10,9,8)]
## multvariate Etasq from fit0
m0 = MultivarMat_DOWNSAM["DP1", c(4:10) ]
m0 = c( m0[1:2], NA, m0[3], NA, m0[4:7], NA)
## multvariate Etasq from fit
m =  MultivarMat_DOWNSAM["DP1", c(20:25) ] 
m = c( NA, m[6], NA, m[5], NA, m[4:3], NA, m[2:1])
...
d = as_tibble(d)
d$labels <- factor(d$labels, levels = d$labels[10:1])
...
p = d %>% ggplot(aes(x = varexp, y = labels)) + 
  ...
  labs(title = paste0("Variance explained in capture sensitivity \n derived from univariate and type I multivariate ANOVAs"),
       ...
       `Average Fragment Size` + `Starting DNA` + `Capture Pool` + `Sequencing Batch`\n\nmodel `fit` has the order of explanatory variables reversed") )
...
```{r, message = FALSE, warning = FALSE, error=FALSE, fig.width = 15, fig.height = 5}
...
p = wdata[-w,] %>% ggplot( aes(x = `Unique Reads`, y = `Capture Sensitivity (CS) DP1`)) +
  ...
  geom_point(aes(fill = `Sequencing Batch`, shape = `Sequencing Batch`,  size = `Library Complexity (LC)`), alpha = 0.8 ) +
  ...
  labs(title = "Associations beteween unique reads and capture sensitiviy @DP1", 
...
```{r, message = FALSE, warning = FALSE, error=FALSE, fig.width = 15, fig.height = 6}

w = which(downdata$`Production Reads` < 1500000 )
######
par(mfrow = c(2,1))
boxplot(wdata$`Unique Reads` ~ wdata$`Capture Pool`)
boxplot(downdata$`Unique Reads`[-w] ~ downdata$`Capture Pool`[-w])
...
```{r, message = FALSE, warning = FALSE, error=FALSE, fig.width = 20, fig.height = 8}
w = which(downdata$`Production Reads` < 1500000)
...
```{r, message = FALSE, warning = FALSE, error=FALSE, message=FALSE, fig.width = 15, fig.height = 10}
#library(network)
library(igraph)
